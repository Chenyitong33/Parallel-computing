...
void worker_pgm(int myID, int width, int height, double real_min, 
	...
	//int *data_msg = (int *)malloc((width+1) * sizeof(int));
	int data_msg[width+1];
  ...
  for (int col = 0; col < width; ++col) {
		...
		double img = imag_min + ( (height-1-the_row) * scale_imag);
	...
	data_msg[col+1] = inside;//inset(real_step, img_step, maxiter);
	}
  
	MPI_Send(data_msg, width+1, MPI_INT, 0, DATA_TAG,
		MPI_COMM_WORLD);
	}
...
int main(int argc, char *argv[]){
	...
	//int *data_msg = (int *)malloc((num+1) * sizeof(int));
	int data_msg[num+1];
  MPI_Status status;
	int tasks_not_done;
	int id;
  ...
  
  {
	...
	while (tasks_not_done > 0) {
		...
		{
		MPI_Recv(data_msg, num+1, MPI_INT, MPI_ANY_SOURCE,
